...
vogels.dynamoDriver = internals.dynamoDriver = driver => {
  if (driver) {
...
vogels.documentClient = internals.documentClient = docClient => {
  if (docClient) {
...
internals.compileModel = (name, schema) => {
  ...
  const log = vogels.log.child({ model: name });
...
vogels.define = (modelName, config) => {
...
vogels.createTables = (options, callback) => {
